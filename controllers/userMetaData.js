...
exports.updateUserMetaData = async (req, res) => {
  try {
    ...
    if (!["username", "email"].every((key) => req.body[key]))
    ...
    let user = await UserMetaData.findOne({
...
exports.subscribeIfExist = async (req, res) => {
  try {
    // validation
    if (!req.headers?.authorization)
      throw Error("Validation Failed: authorization isn't set");
    ...
    // is user provided info before or does email or username exist?
    let userMeta = await UserMetaData.findOne({
      walletAddress: address,
    });

    if (!userMeta || (userMeta && userMeta?.isDefault)) {
      return res.status(200).send({
        status: "error",
      });
    }
...
//         completedTime:
//           new Date(user?.lastBestGuessTimestamp).getTime() -
//           gameStartedAtInSeconds * 1000,
...
//           lastBestGuessTimestamp: gameStartedAtInSeconds,
//           lastNoOfCorrectSeedsOrInOrderTimestamp: gameStartedAtInSeconds,
//           lastTriedTimestamp: gameStartedAtInSeconds,
//         });
//         await user.save();
//       }
//     }
